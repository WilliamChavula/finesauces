...
from .tasks import status_change_notofication
import xlsxwriter
import datetime
...
def export_to_xlsx(modelAdmin, request, queryset):
    ...
    for row, obj in enumerate(queryset):
        ...
        # iterate through order items data - qty and price
        # and append to data_row_list
...
def status_change(queryset, status):
    for order in queryset:
        ...
        status_change_notofication(order.id)
...
status_processing.short_description = "status_shipped"
...
status_processing.short_description = "status_ready_for_pickup"
